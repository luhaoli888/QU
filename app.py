...
def process_data_logic(file):
    ...
    for col in ['修复胜率', '登场率']:
        if col in df_main.columns and df_main[col].dtype == 'object':
            df_main[col] = df_main[col].astype(str).str.replace('%', '', regex=False).astype(float)
    if 'Ban率' in df_ban.columns and df_ban['Ban率'].dtype == 'object':
        df_ban['Ban率'] = df_ban['Ban率'].astype(str).str.replace('%', '', regex=False).astype(float)
    
    df_full = pd.merge(df_main, df_ban, on=['英雄名', 'MMR'], how='left').dropna(subset=['Ban率'])
